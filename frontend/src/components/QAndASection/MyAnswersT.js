...
import { getAnswerT } from '../../api/apiFBQA';
// import myAnswerBg from './ImagesD/myAnswerBg.png'

export const MyAnswersT = () => {
    const match = useRouteMatch()
    const [answer, setAnswer] = useState([]);

    useEffect(()=>{
        const displayAnswer = async()=>{
            const answer = await getAnswerT(match.params.username)
            setAnswer(answer);
        }
        displayAnswer();
    },[])
                ...
                <tbody>
                    {
                        ...
                        <tr>
                            <td>{row.qTopic}</td>
                            <td>
                                <Link to={`/employee/updateA/${row._id}`} style={{ textDecoration: 'none' }}><Button style={btn} color="Secondary" variant="contained">Update Answer</Button></Link>
                                <Link to={`/employee/deleteA/${row._id}`} style={{ textDecoration: 'none', marginLeft:'10pt'}}><Button style={btn} color="Secondary" variant="contained">Delete Answer</Button></Link>
                            </td>
                        </tr>                      
                        ))
                    }
